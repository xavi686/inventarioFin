...
class VerificarExistencia{
    ...
    public VerificarExistencia(Connection connection, String grupo, int caja, int num_conteo) {
    ...
    public VerificarExistencia(Connection connection, String grupo, int num_conteo) {
        this.connection = connection;
        this.grupo = grupo;
        this.num_conteo = num_conteo;
    }
...
public class CodigoBarrasDB2 {
    ...
    public static VerificarExistencia obtenerInfo(Connection connection, String grupo, int caja) {
    	int num_conteo = 1;
        String conteo_query = "SELECT grupo, caja, num_conteo FROM conteo WHERE caja = " + caja
        		+ " GROUP BY grupo, caja, num_conteo";
    	...
    	return new VerificarExistencia(connection, grupo, num_conteo);
    }
    	...
    	try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
	        ...
	        while (continuar_principal) {
                ...
                case 1:
                    ...
                    String grupo = scanner.nextLine().toUpperCase().trim();
                    System.out.print("Ingrese el número de la caja: "); 
                    //Verificar si algún grupo ya leyó la caja ingresada
                    int caja = scanner.nextInt();
//                    num_conteo = verificar_conteo(connection, grupo, caja);
                    VerificarExistencia verifica = obtenerInfo(connection, grupo, caja);
                    boolean continuar = true;
                    if(grupo.equals(verifica.getGrupo())) {
...
//Sub menu para continuar con el conteo
                    ...
                    case 2:
                        // Ingreso datos de la caja y grupo
                        System.out.print("Ingrese el grupo: "); //Preferible el mismo grupo
                        grupo = scanner.nextLine().toUpperCase().trim();
                        System.out.print("Ingrese el número de la caja: "); 
                        //Verificar si algún grupo ya leyó la caja ingresada
                        caja = scanner.nextInt();
                        //num_conteo = verificar_conteo(connection, grupo, caja);
                        verifica = obtenerInfo(connection, grupo, caja);
                        num_conteo = verifica.getNum_conteo();
                        if(grupo.equals(verifica.getGrupo())){
                            scanner.nextLine();
                            codigobarra = leercodigo(scanner);
                            proceso_recontar(connection, codigobarra, grupo, caja, num_conteo);
                            continuar = true;
                	        while (continuar) {
                	        	System.out.println("Seleccione la opción a realizar en reconteo:");
                	        	System.out.println("1. Seguir contando");
                	        	System.out.println("2. Imprimir reporte");
                	        	System.out.println("3. Finalizar conteo");
                	        	System.out.print("Ingrese su opción: ");
                            ...
                            System.out.println("La caja debe ser contada nuevamente por el grupo " + verifica.getGrupo());
                            System.out.println("Por favor tomar otra caja!");
                        }
                    	break;
                    case 3:
                        //Consolidado
...
//Con este sólo guardo los primeros 16 caracteres del código de barras
    ...
    private static void proceso_recontar(Connection connection, String codigobarra, String grupo, int caja, 
    		int num_conteo){
        ...
        Scanner scanner = new Scanner(System.in);
        System.out.print("Ingrese la cantidad contada: "); //A, B o C
        int total = scanner.nextInt();
        ...
        try {
            ...
            if (encontrado==true) {
//Con este sólo guardo los primeros 16 caracteres del código de barras
            	reconteo(connection, codigo, lote, itemcode, referencia, fecha, total, cantidad, grupo, caja, num_conteo);
            }
    ...
    private static String leercodigo(Scanner scanner) {
    	                    ...
    	                    String codigobarra = scanner.nextLine().trim();
    	                    return codigobarra;
    }
    ...
    private static void reconteo(Connection connection, String codebars, String batchnum, String itemcode,
        ...
        String conteo_query = "SELECT caja, num_conteo FROM conteo WHERE caja = " + caja
        ...
        String query = "INSERT INTO conteo (codebars, batchnum, itemcode, referencia, "
        		+ "vencimiento, total, cantidad, grupo, caja, "
        		+ "fecha, num_conteo, hora) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        num_conteo++; //incrementa el conteo de la caja al hacer reconteo
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setString(1, codebars);
            statement.setString(2, batchnum);
            statement.setString(3, itemcode);
    ...
    //Opcional en caso de que en la opción de conteo nuevo falle
    private static int verificar_conteo(Connection connection, String grupo, int caja){
    	int num_conteo = 1;
        String conteo_query = "SELECT grupo, caja, num_conteo FROM conteo WHERE caja = " + caja
        		+ " GROUP BY grupo, caja, num_conteo";
        try (Statement statement = connection.createStatement();
